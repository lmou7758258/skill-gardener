...
- **模型检查的前提**：断粮检查优先读取 `cron/jobs.json` 里目标巡检 job 实际 pin 的 `model`/`provider`；只有 job 没写 `model` 时才回退 `config.yaml` 默认模型。该检查假设你用的是 OpenAI 兼容中转站（暴露 `GET /v1/models`，且目标模型在该清单里）。若用大模型供应商官方 API（OpenAI/Anthropic/Gemini/xAI）或自建非标准 gateway，此项会误报「不可达/断粮」——设环境变量 `SKILL_GARDENER_SKIP_MODEL_CHECK=1` 可跳过它，job 健康与超期检查照常。
