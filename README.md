...
> **The model check targets the cron job's actual pinned model.** The watchdog reads `model`/`provider` from the named job in `cron/jobs.json`; it only falls back to `config.yaml`'s default model when that job has no model field. The check assumes an OpenAI-compatible relay: it calls `GET /v1/models` and expects the target model in that list. If you use a **first-party model provider** (OpenAI, Anthropic, Gemini, xAI, …) or a **custom/self-hosted gateway** without a standard `/v1/models`, this check will false-alarm. Set `SKILL_GARDENER_SKIP_MODEL_CHECK=1` to disable just that check — job-health and staleness checks still run.
...
- **Watchdog assumptions**: `watchdog.py` expects an OpenAI-compatible relay exposing `GET /v1/models`. It checks the target cron job's pinned `model`/`provider` from `cron/jobs.json` first, falling back to `config.yaml`'s default model only when the job has no `model`. A different layout will surface as a parse/configuration error — that's the parser's assumption, not a relay outage.
